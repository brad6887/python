...
def box_ok(box):
    ...
    Return True if the heights in the box oare in nondecreasing order,
    False otherwise.
    """
...
def all_boxes_ok(boxes):
    ...
    Return True if each box in boxes has it action figures in
...
def boxes_endpoints(boxes):
    ...
    Return a list of boxes; where each value is a list of two values:
    the heights of the leftmost and rightmost action fiugres in a box.
    """
    endpoints=[]
    for box in boxes:
...
def all_endpoints_ok(endpoints):
    ...
    Require: endpoints is sorted by action figure heights.
...
if not all_boxes_ok(boxes):
    ...
    # Determine if boxes are organized
