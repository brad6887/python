...
Fermi   Onde digit is correct and in the right position.
...
clues would be Fermi Pico.'''.format(NUM_DIGITS))
    ...
    # This stores secret number the player needs to guess
...
def getClues(guess, secretNum):
    """Returns a string with the pico, fermi, bages clues for a guess
    ...
    if len(clues) == 0:
        ...
        # Sort the clues inot alphabetical orderso their original order
...
message = "c"   # global variable, avoid when possible

def greet():
    message = "a"   # Local to function
    print(message)  # prints 'a'

def send_email(name):
    message = "b"   # same varible can be used locally
    print(message)  # prints 'b'

print(message)      # prints 'c'
