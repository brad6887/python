...
def no_high(lst):
    ...
    Return True if ther are no high cards in lst, False otherwise
    """
    if 'jack' in lst:
        return False
    if 'queen' in lst:
...
for i in range(NUM_CARDS):
    ...
    if card == 'jack' and remaining >= 1 and no_high(deck[i+1:1+2]):
        points = 1
    elif card == 'queen' and remaining >= 2 and no_high(deck[i+1:i+3]):
        points = 2
    elif card == 'king' and remaining >= 3 and no_high(deck[i+1:i+4]):
        points = 3
    elif card == 'king' and remaining >= 4 and no_high(deck[i+1:i+5]):
        points = 4
        
    if points > 0:
        print(f'Player {player} scores {points} points(s).')
